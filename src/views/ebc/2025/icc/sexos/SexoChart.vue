...
const getSexoColor = (item, index, fallbackColors) => {
  const sexo = sexos.find(
    (sexoItem) =>
      Number(sexoItem.id) === Number(item.codigo) ||
      normalizeText(sexoItem.nombre) === normalizeText(item.nombre),
  )

  return sexo?.color || fallbackColors[index % fallbackColors.length]
}
...
const initChart = () => {
  ...
  const data = dataForChart.map((d, index) => ({
    ...
    color: getSexoColor(d, index, fallbackColors),
  }))
